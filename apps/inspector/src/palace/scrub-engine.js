...
export function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    if (near !== activeIndex) {
      ...
      // Host-app hook (added for the inspector): lets React swap the live room
      // console when the camera reaches a new room, without polling scroll math.
      container.dispatchEvent(new CustomEvent('sw:section', { detail: { index: near, id: SECTIONS[near].id } }));
    }
