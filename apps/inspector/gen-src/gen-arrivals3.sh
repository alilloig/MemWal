...
# room stills. 2 at a time (plan concurrency). Encodes on success.
cd "$(dirname "$0")"
...
one() {
  ...
  local url=$(python3 -c "import json;d=json.load(open('arr2_${name}.json'));i=d[0] if isinstance(d,list) else d;print(i.get('result_url') or '')" 2>/dev/null)
  if [ -n "$url" ]; then
